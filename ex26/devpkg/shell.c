...
int Shell_exec(Shell template, ...) {
    ...
    check(replaced == template.args_to_replace, "Failed to substitute all parameters.");
...
int Shell_run(apr_pool_t *p, Shell *cmd) {
    ...
    check(rv == APR_SUCCESS, "Failde to set cmd type.");
...
Shell CLEANUP_SH = {
    .exe = "rm",
    .dir = "/tmp",
    .args = {"rm", "-rf", "/tmp/pkg-build/", "/tmp/pkg-src.tar.gz", "/tmp/pkg-src.tar.bz2", 
        "/tmp/DEPENDS", NULL},
    .args_to_replace = 0
};
