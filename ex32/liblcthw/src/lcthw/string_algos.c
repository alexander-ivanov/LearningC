...
int String_find(bstring in, bstring what) {
    ...
    const unsigned char *heystack = (const unsigned char *)bdata(in);
    ...
    found = String_base_search(heystack, hlen, needle, nlen, skip_chars);

    return found != NULL ? found - heystack : -1;
}
...
StringScanner *StringScanner_create(bstring in) {
    ...
    assert(scan != NULL && "Failed to create scan.");
    return scan;
