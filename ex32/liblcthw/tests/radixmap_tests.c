...
static int check_order(RadixMap *map) {
    ...
    // only signal errors if and (should not be)
...
static char *test_operations() {
    ...
    RadixMap_sort(map);
    mu_assert(check_order(map), "Failed to properly sort the RedixMap.");
    ...
    while (map->end > 0) {
        ...
        //test the the end is now the old value, but uint32 max so it trails off
